...
from modules.comp_fingerprint import *
...
def get_family(family, specie):
    if len(specie) > 0:
        result = (
            NaturalProducts.objects.all()
            .filter(family=family, specie_1=specie)
            .values(
                "ID",
                "family",
                "specie_1",
...
def get_chemical_information(form):
    result = NaturalProducts.objects.exclude(mw__lt=form.mw_lt).exclude(
        mw__gt=form.mw_gt
    )
    ...
    # lipinsky
    result = result.exclude(lipinsky__lt=form.lipinsky_lt).exclude(
        lipinsky__gt=form.lipinsky_gt
    )
...
def get_similar_compounds(smiles):
    result = NaturalProducts.objects.all()
    smiles_from_database = list(map(lambda x: result[x].smiles, range(len(result))))
    # smiles_from_database = smiles_from_database[:5]
    fp_list, cleaned_smiles = maccskeys_fp(smiles_from_database)
    fp_target = target_mol_fp_representation(smiles)
    sim = get_similarity(fp_target, fp_list)
    similar_compounds = get_similar_comp(sim, cleaned_smiles)
    print("SIMILAR_COMPOUNDS", similar_compounds)
    # ejecutar nueva query
    final_data = pd.DataFrame(columns=["ID", "Family"])
    for i in range(len(similar_compounds)):
        result = NaturalProducts.objects.filter(smiles=similar_compounds[i]).values(
            "ID",
            "family",
            # "specie_1",
            # "specie_2",
